...
function unit_commitment(buses_df::DataFrame, demand_df::DataFrame, generators_df::DataFrame, lines_df::DataFrame, renewable_df::DataFrame, Interval::DataFrame)
    ...
    set_optimizer_attribute(model, "MIPGap", 0.01)  # Ajusta el gap a 0.1%
    ...
    RESup = Dict(t => (Interval."Upper"[t] - Interval."Prom"[t]) for t in T)
    RESdown = Dict(t => (Interval."Prom"[t] - Interval."Lower"[t]) for t in T)
    ...
    for t in T
        @constraint(model, sum((pot_min[g] * w[g, t]) for g in G if !is_renewable[g]) <= sum(demand[n, t] for n in N) - RESdown[t])
    end
